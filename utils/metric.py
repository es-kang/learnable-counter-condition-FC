...
def prediction_LR(l):
    l_arr = torch.from_numpy(np.array([b for a in l for b in a]))
    prob = torch.clone(l_arr)
    pred = torch.clone(l_arr)
    pred[pred > 0.5] = 1
    pred[pred < 0.5] = 0
    return pred, prob
...
def cal_all_metric_LR(logit, grou):
    pred, prob = prediction_LR(logit)
    grou = np.array([b for a in grou for b in a]).astype(np.float64)
...
def cal_all_metric_site(logit, grou):
    pred, prob = prediction(logit)
    grou = np.hstack(grou).astype(np.float64)
    pred = pred.detach().cpu().numpy().astype(np.float64)
    prob = prob.detach().cpu().numpy().astype(np.float64)
